...
var MozillaFreshBugs = function MozillaFreshBugs() {};
...
var makeRequest = function makeRequest(urlPath, callback) {
  var options = {
...
var printResult = function printResult(bugs) {
  var separator = '   ';
  ...
  bugs = _(bugs).chain().sortBy('last_change_time').reverse().value();

  bugs.forEach(function (bug) {
    console.log([bug.summary, bug.last_change_time, 'comments: ' + bug.comments.length, 'history: ' + bug.history.length].join(separator));
    console.log();
  });
};
...
var path = generateURL({
  base: '/rest/bug',
  product: '%20Firefox',
  args: [{ name: 'f1', value: 'bug_mentor' }, { name: 'o1', value: 'isnotempty' }, { name: 'whiteboard_type', value: 'contains_all' }],
  bug_status: ['NEW', 'ASSIGNED', 'REOPENED', 'UNCONFIRMED'],
  include_fields: ['id', 'assigned_to', 'summary', 'last_change_time', 'comments']
});

var getUnassignedBugs = function getUnassignedBugs(data) {
  bugs = data.bugs || [];
...
var addBug = function addBug(bug, comments, history) {
...
var getBugDetails = function getBugDetails(bug) {
...
var processBugs = function processBugs(bugs) {
  printResult(bugs);
};
